import { fetchBankTransactionsDetail } from '@app/apis/admin/bank-transaction';
import { Card } from '@app/components/card';
import {
  DataColumn,
  DataTable,
  PageSizeSelect,
  PageSizeSelectChangeFunc,
  PaginatedButtonClickFunc,
  Pagination,
  RenderColumnFunc,
  SortChangeFunc,
} from '@app/components/data-table';
import { DetailDataTable } from '@app/components/detail-data-table/BankTransactionDetailDataTable';
import { DatePicker, FormGroup, Input, Label } from '@app/components/form';
import { PageContainer, PageHeader, PageTitle } from '@app/components/page';
import { SearchCard } from '@app/components/search-card';
import { useForm } from '@app/hooks';
import { AppState } from '@app/store';
import { Alert } from '@modules/alert';
import { useNavigation } from '@modules/router';
import { DEFAULT_PAGE, PAGE_SIZE_OPTIONS } from '@shared/constants';
import { bankTransaction, bankTransactionDetail } from '@shared/types';
import axios from 'axios';
import { check } from 'prettier';
import React, { ChangeEvent, FormEvent, useCallback, useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import {
  fetchBankTransactionsActionCreator,
  hideModalActionCreator,
  resetPageStateActionCreator,
  exportExcelActionCreator,
  parseCsvActionCreator,
  exportOdsActionCreator,
} from './actions';

import './bank-transaction-page.scss';
/** DataTable 欄位名稱 */
export type bankTransactionTableField = keyof (bankTransaction & {
  actions: string;
});

/** SearchCard Form */
export type SearchCardForm = {
  /** 身分證帳號 */
  idCardNumber: string;
  /** 交易帳號 */
  transactionAccountId: string;
  /** 交易行 */
  transactionBank: string;
  /** 交易摘要 */
  transactionSummary: string;
  /** 交易日期(起) */
  transactionTimeStart: string;
  /** 交易日期(迄) */
  transactionTimeEnd: string;
};

/** 「模板頁面」元件 */
export const BankTransactionPage = () => {
  //#region Hooks

  /** DataTable 欄位設定 */
  const [dataTableMasterColumns] = useState([
    { field: 'actions', name: '', isSortable: false },
    { field: 'idCardNumber', name: '身分證字號' },
    { field: 'bankName', name: '銀行名稱' },
    { field: 'bankBranchName', name: '分行名稱' },
    { field: 'transactionAccountId', name: '帳號' },
    { field: 'createTime', name: '資料匯入時間' },
  ]);

  const [dataTableContainer] = useState([{ field: 'actions', name: '' }]);

  /** SearchCard Form 欄位資料 */
  const { formData, updateFormData } = useForm<SearchCardForm>({
    idCardNumber: { initialValue: '', validate: () => ({}) },
    transactionAccountId: { initialValue: '', validate: () => ({}) },
    transactionBank: { initialValue: '', validate: () => ({}) },
    transactionSummary: { initialValue: '', validate: () => ({}) },
    transactionTimeStart: { initialValue: '', validate: () => ({}) },
    transactionTimeEnd: { initialValue: '', validate: () => ({}) },
  });

  /** 當前取得帳號資料的搜尋條件資料 */
  const currentFetchBankTransactionParams = useSelector(
    (state: AppState) => state.pages.bankTransaction.currentFetchBankTransactionParams,
  );

  /** 取得案號 */
  const caseno = useSelector((state: AppState) => state.userFile.userFile);

  /** 取得案名 */
  const casename = useSelector((state: AppState) => state.userFile.userFileNameOptions).filter((x) => {
    return x.value == caseno;
  })[0].text;

  /** 帳號資料 */
  const bankTransaction = useSelector((state: AppState) => state.pages.bankTransaction.bankTransactionLists);

  /** 分頁資訊資料 */
  const paginatedInfo = useSelector((state: AppState) => state.pages.bankTransaction.paginatedInfo);

  /** 是否正在取得帳號資料 */
  const isFetchBankTransactionsLoading = useSelector(
    (state: AppState) => state.pages.bankTransaction.isFetchBankTransactionsLoading,
  );

  /** Redux dispatch function */
  const dispatch = useDispatch();

  /** Navigation */
  const navigation = useNavigation();

  //#endregion Hooks

  /** 搜尋按鈕事件 */
  const handleSearchFormSubmit = useCallback(
    (e: FormEvent) => {
      e.preventDefault();
      dispatch(
        fetchBankTransactionsActionCreator({
          ...currentFetchBankTransactionParams,
          ...formData,
          page: DEFAULT_PAGE,
        }),
      );
    },
    [dispatch, formData, currentFetchBankTransactionParams],
  );

  /** 處理分頁大小變換事件 */
  const handlePageSizeChange: PageSizeSelectChangeFunc = useCallback(
    (pageSize) => {
      dispatch(
        fetchBankTransactionsActionCreator({
          ...currentFetchBankTransactionParams,
          page: DEFAULT_PAGE,
          pageSize: Number(pageSize),
        }),
      );
    },
    [dispatch, currentFetchBankTransactionParams],
  );

  /** 處理 DataTable 欄位排序變換事件 */
  const handleSortedColumnChange: SortChangeFunc = useCallback(
    (field, sortedType) => {
      dispatch(
        fetchBankTransactionsActionCreator({
          ...currentFetchBankTransactionParams,
          page: DEFAULT_PAGE,
          sortedType,
          sortedColumn: field,
        }),
      );
    },
    [dispatch, currentFetchBankTransactionParams],
  );

  /** 處理 DataTable 換頁事件 */
  const handlePageChange: PaginatedButtonClickFunc = useCallback(
    (nextPage) => {
      dispatch(
        fetchBankTransactionsActionCreator({
          ...currentFetchBankTransactionParams,
          page: nextPage,
        }),
      );
    },
    [dispatch, currentFetchBankTransactionParams],
  );

  /** 匯出開戶資料excel */
  const exportBankTransactionexcel = useCallback(() => {
    dispatch(exportExcelActionCreator({ ...currentFetchBankTransactionParams }));
  }, [dispatch, currentFetchBankTransactionParams]);

  /** 匯出開戶資料ods */
  const exportBankTransactionods = useCallback(() => {
    dispatch(exportOdsActionCreator({ ...currentFetchBankTransactionParams }));
  }, [dispatch, currentFetchBankTransactionParams]);
  /** 上傳CSV事件 */
  const handleUploadCsv = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      e.preventDefault();
      if (e.target.files && e.target.files.length) {
        const file = e.target.files[0];
        const formData = new FormData();
        formData.append('file', file);
        formData.append('caseno', caseno);
        formData.append('casename', casename);
        dispatch(parseCsvActionCreator(formData));
        e.target.value = '';
      }
    },
    [dispatch, caseno, casename],
  );

  /** Render DataTable Columns */
  const renderColumn: RenderColumnFunc<bankTransaction, bankTransactionTableField> = ({ field, data }) => {
    if (field === 'actions') {
      return (
        <DataColumn>
          <div
            className="accordion-button collapsed"
            data-bs-toggle="collapse"
            data-bs-target={`#expand${data.transactionAccountId}`}
            aria-expanded="false"
            aria-controls={`expand${data.transactionAccountId}`}
            style={{ backgroundColor: '#dee2e6', boxShadow: 'none' }}
          ></div>
        </DataColumn>
      );
    }
    return <DataColumn>{data[field]}</DataColumn>;
  };

  const renderColumnContainer: RenderColumnFunc<bankTransaction, bankTransactionTableField> = ({ field, data }) => {
    if (field === 'actions') {
      return (
        <DataColumn>
          <div className="accordion-item">
            <div className="accordion-header">
              <DataTable
                columns={dataTableMasterColumns}
                data={[data]}
                onSortChange={handleSortedColumnChange}
                sortedColumn={currentFetchBankTransactionParams.sortedColumn}
                sortedType={currentFetchBankTransactionParams.sortedType}
                renderColumn={renderColumn}
                keyExtractor={(item: bankTransaction) => item.transactionAccountId}
                isLoading={isFetchBankTransactionsLoading}
                isHasHead={bankTransaction[0].transactionAccountId == data.transactionAccountId ? true : false}
                datatableTrColor={'#dee2e6'}
              />
            </div>
            <DetailDataTable
              id={data.transactionAccountId}
              parentid={'BankTransactionaccordion'}
              isFetch={true}
              searchParams={currentFetchBankTransactionParams}
            ></DetailDataTable>
          </div>
        </DataColumn>
      );
    }
  };

  //#region Effects
  useEffect(() => {
    // 載入畫面時，取得帳號列表
    dispatch(fetchBankTransactionsActionCreator(currentFetchBankTransactionParams));

    return () => {
      // 重置頁面 State
      dispatch(resetPageStateActionCreator());
    };
  }, [dispatch]);

  //#endregion Effects

  return (
    <PageContainer>
      <PageHeader>
        <PageTitle>銀行交易明細</PageTitle>
      </PageHeader>

      {/* Search Card */}
      <SearchCard>
        <form className="container-fluid" onSubmit={handleSearchFormSubmit}>
          <div className="row">
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="idCardNumber">身分證字號</Label>
                <Input
                  type="text"
                  id="idCardNumber"
                  name="idCardNumber"
                  onChange={(e) => updateFormData({ idCardNumber: e.target.value })}
                  value={formData.idCardNumber}
                  disabled={isFetchBankTransactionsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="transactionAccountId">交易帳號</Label>
                <Input
                  type="text"
                  id="transactionAccountId"
                  name="transactionAccountId"
                  onChange={(e) => updateFormData({ transactionAccountId: e.target.value })}
                  value={formData.transactionAccountId}
                  disabled={isFetchBankTransactionsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="transactionBank">交易行</Label>
                <Input
                  type="text"
                  id="transactionBank"
                  name="transactionBank"
                  onChange={(e) => updateFormData({ transactionBank: e.target.value })}
                  value={formData.transactionBank}
                  disabled={isFetchBankTransactionsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="transactionSummary">交易摘要</Label>
                <Input
                  type="text"
                  id="transactionSummary"
                  name="transactionSummary"
                  onChange={(e) => updateFormData({ transactionSummary: e.target.value })}
                  value={formData.transactionSummary}
                  disabled={isFetchBankTransactionsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="transactionTimeStart">交易日期(起)</Label>
                <DatePicker
                  id="transactionTimeStart"
                  name="transactionTimeStart"
                  onChange={(e) => updateFormData({ transactionTimeStart: e })}
                  value={formData.transactionTimeStart}
                  disabled={isFetchBankTransactionsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="transactionTimeEnd">交易日期(迄)</Label>
                <DatePicker
                  id="transactionTimeEnd"
                  name="transactionTimeEnd"
                  onChange={(e) => updateFormData({ transactionTimeEnd: e })}
                  value={formData.transactionTimeEnd}
                  disabled={isFetchBankTransactionsLoading}
                />
              </FormGroup>
            </div>
          </div>
          <div className="row">
            <div className="col-12 text-center">
              <button type="submit" className="btn btn-primary btn-width-xlg" disabled={isFetchBankTransactionsLoading}>
                交易明細查詢
              </button>
            </div>
          </div>
        </form>
      </SearchCard>

      {/* Table Card */}
      <Card>
        <Card.Header>
          <PageSizeSelect options={PAGE_SIZE_OPTIONS} onChange={handlePageSizeChange} value={String(paginatedInfo.pageSize)} />
          <div>
            <input id="uploadFile" type="file" className="btn-check" onChange={handleUploadCsv}></input>
            <label className="btn btn-primary btn-width-lg" htmlFor="uploadFile" style={{ marginRight: '15px' }}>
              <i className="fas fa-upload"></i> .CSV資料匯入
            </label>
            <img
              src="data:image/svg+xml;base64,"
              className="jsx-2310615273"
              onClick={exportBankTransactionexcel}
              style={{ width: '40px', height: '40px', marginRight: '10px', cursor: 'pointer' }}
            ></img>
            <img
              src="data:image/svg+xml;base64,"
              className="jsx-2310615273"
              onClick={exportBankTransactionods}
              style={{ width: '40px', height: '40px', marginRight: '10px', cursor: 'pointer' }}
            ></img>
          </div>
        </Card.Header>
        <Card.Body>
          <div className="accordion" id="BankTransactionaccordion">
            <DataTable
              columns={dataTableContainer}
              data={bankTransaction}
              onSortChange={handleSortedColumnChange}
              sortedColumn={currentFetchBankTransactionParams.sortedColumn}
              sortedType={currentFetchBankTransactionParams.sortedType}
              renderColumn={renderColumnContainer}
              keyExtractor={(item: bankTransaction) => item.transactionAccountId}
              isLoading={isFetchBankTransactionsLoading}
            />
          </div>
        </Card.Body>
        <Card.Footer>
          <Pagination
            page={paginatedInfo.page}
            totalPage={paginatedInfo.totalPage}
            pageSize={paginatedInfo.pageSize}
            totalCount={paginatedInfo.totalCount}
            onPaginatedButtonClick={handlePageChange}
            disabled={isFetchBankTransactionsLoading}
          />
        </Card.Footer>
      </Card>
    </PageContainer>
  );
};
