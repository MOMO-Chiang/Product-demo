...
import { fetchCreateTransactionSearchActionCreator } from '../actions';
import { ActionUserModalForm } from '../CryptoTransactionInfoPage';
...
export default function OtherActionUserModal(from: any) {
  ...
  const handleCreateSearch = useCallback(
    (e: FormEvent) => {
      ...
      dispatch(
        fetchCreateTransactionSearchActionCreator({
  ...
  /** 若拋查成功，關閉modal */
  useEffect(() => {
    if (isCreateTransactionInfoSearchSuccess) {
      setShow(false);
    }
  }, [isCreateTransactionInfoSearchSuccess]);
  //#endregion Effects
  return (
    <>
      <button onClick={handleShow} className="btn btn-primary btn-width-lg" disabled={isFetchCryptoTransactionInfosLoading}>
        開啟代拋查
      </button>
