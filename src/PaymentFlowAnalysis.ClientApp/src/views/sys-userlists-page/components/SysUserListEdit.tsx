...
export const SysUserListEdit = () => {
  ...
  /** 是否成功撈取資料 */
  const isFetchSysUserListSuccess = useSelector((state: AppState) => state.pages.sysUserLists.isFetchSysUserListSuccess);

  /** 當前的資料 id */
  const currentModalUserId = useSelector((state: AppState) => state.pages.sysUserLists.currentModalUserId) as string;

  /** 當前的資料 */
  const currentModalSysUserList = useSelector((state: AppState) => state.pages.sysUserLists.currentModalSysUserList);
  ...
  /** 初始撈取帳號資料 */
  useEffect(() => {
    dispatch(fetchSysUserListActionCreator(currentModalUserId));
