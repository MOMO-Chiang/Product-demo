import { Card } from '@app/components/card';
import {
  DataColumn,
  DataTable,
  PageSizeSelect,
  PageSizeSelectChangeFunc,
  PaginatedButtonClickFunc,
  Pagination,
  RenderColumnFunc,
  SortChangeFunc,
} from '@app/components/data-table';
import { DatePicker, FormGroup, Input, Label } from '@app/components/form';
import { PageContainer, PageHeader, PageTitle } from '@app/components/page';
import { SearchCard } from '@app/components/search-card';
import { useForm } from '@app/hooks';
import { AppState } from '@app/store';
import { Alert } from '@modules/alert';
import { useNavigation } from '@modules/router';
import { DEFAULT_PAGE, PAGE_SIZE_OPTIONS } from '@shared/constants';
import { bankSafeDepositBox } from '@shared/types';
import React, { ChangeEvent, FormEvent, useCallback, useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import {
  hideModalActionCreator,
  resetPageStateActionCreator,
  exportExcelActionCreator,
  parseCsvActionCreator,
  exportOdsActionCreator,
  fetchBankSafeDepositBoxsActionCreator,
  isAccountMarkActionCreator,
} from './actions';

/** DataTable 欄位名稱 */
export type bankSafeDepositBoxTableField = keyof bankSafeDepositBox;

/** SearchCard Form */
export type SearchCardForm = {
  /** 身分證帳號 */
  idCardNumber: string;
  /** 承租人 */
  renter: string;
  /** 行動電話 */
  mobilePhone: string;
  /** 箱號或室號 */
  boxNumber: string;
  /** 承租日期(起) */
  rentDateStart: string;
  /** 承租日期(迄) */
  rentDateEnd: string;
  /** 退租日期(起) */
  leaseCancellationDateStart: string;
  /** 退租日期(迄) */
  leaseCancellationDateEnd: string;
  /** 關鍵字 */
  keyWord: string;
  /** 是否顯示本案相關帳戶 */
  isAccountMark: string;
};

/** 「模板頁面」元件 */
export const BankSafeDepositBoxPage = () => {
  //#region Hooks

  /** DataTable 欄位設定 */
  const [dataTableColumns] = useState([
    { field: 'isAccountMark', name: '本案相關帳戶', isSortable: false },
    { field: 'idCardNumber', name: '身分證字號' },
    { field: 'bankBranchCode', name: '出租行總分支機構代碼' },
    { field: 'boxRentType', name: '承租種類' },
    { field: 'renter', name: '承租人' },
    { field: 'localPhone', name: '市內電話' },
    { field: 'mobilePhone', name: '行動電話' },
    { field: 'residenceAddress', name: '戶籍地址' },
    { field: 'mailingAddress', name: '通訊地址' },
    { field: 'boxNumber', name: '箱號或室號' },
    { field: 'dataProvidedTime_Cov', name: '資料提供日' },
    { field: 'rentDate_Cov', name: '承租日' },
    { field: 'leaseCancellationDate_Cov', name: '退租日' },
    { field: 'remark', name: '備註' },
  ]);

  /** SearchCard Form 欄位資料 */
  const { formData, updateFormData } = useForm<SearchCardForm>({
    idCardNumber: { initialValue: '', validate: () => ({}) },
    renter: { initialValue: '', validate: () => ({}) },
    mobilePhone: { initialValue: '', validate: () => ({}) },
    boxNumber: { initialValue: '', validate: () => ({}) },
    rentDateStart: { initialValue: '', validate: () => ({}) },
    rentDateEnd: { initialValue: '', validate: () => ({}) },
    leaseCancellationDateStart: { initialValue: '', validate: () => ({}) },
    leaseCancellationDateEnd: { initialValue: '', validate: () => ({}) },
    keyWord: { initialValue: '', validate: () => ({}) },
    isAccountMark: { initialValue: '', validate: () => ({}) },
  });

  /** 當前取得帳號資料的搜尋條件資料 */
  const currentFetchBankSafeDepositBoxParams = useSelector(
    (state: AppState) => state.pages.bankSafeDepositBox.currentFetchBankSafeDepositBoxParams,
  );

  /** 帳號資料 */
  const bankSafeDepositBox = useSelector((state: AppState) => state.pages.bankSafeDepositBox.bankSafeDepositBoxLists);

  /** 分頁資訊資料 */
  const paginatedInfo = useSelector((state: AppState) => state.pages.bankSafeDepositBox.paginatedInfo);

  /** 是否正在取得帳號資料 */
  const isFetchBankSafeDepositBoxsLoading = useSelector(
    (state: AppState) => state.pages.bankSafeDepositBox.isFetchBankSafeDepositBoxsLoading,
  );

  /** Redux dispatch function */
  const dispatch = useDispatch();

  /** Navigation */
  const navigation = useNavigation();

  //#endregion Hooks

  /** 搜尋按鈕事件 */
  const handleSearchFormSubmit = useCallback(
    (e: FormEvent) => {
      e.preventDefault();
      dispatch(
        fetchBankSafeDepositBoxsActionCreator({
          ...currentFetchBankSafeDepositBoxParams,
          ...formData,
          page: DEFAULT_PAGE,
        }),
      );
    },
    [dispatch, formData, currentFetchBankSafeDepositBoxParams],
  );

  /** 處理分頁大小變換事件 */
  const handlePageSizeChange: PageSizeSelectChangeFunc = useCallback(
    (pageSize) => {
      dispatch(
        fetchBankSafeDepositBoxsActionCreator({
          ...currentFetchBankSafeDepositBoxParams,
          page: DEFAULT_PAGE,
          pageSize: Number(pageSize),
        }),
      );
    },
    [dispatch, currentFetchBankSafeDepositBoxParams],
  );

  /** 處理 DataTable 欄位排序變換事件 */
  const handleSortedColumnChange: SortChangeFunc = useCallback(
    (field, sortedType) => {
      dispatch(
        fetchBankSafeDepositBoxsActionCreator({
          ...currentFetchBankSafeDepositBoxParams,
          page: DEFAULT_PAGE,
          sortedType,
          sortedColumn: field,
        }),
      );
    },
    [dispatch, currentFetchBankSafeDepositBoxParams],
  );

  /** 處理 DataTable 換頁事件 */
  const handlePageChange: PaginatedButtonClickFunc = useCallback(
    (nextPage) => {
      dispatch(
        fetchBankSafeDepositBoxsActionCreator({
          ...currentFetchBankSafeDepositBoxParams,
          page: nextPage,
        }),
      );
    },
    [dispatch, currentFetchBankSafeDepositBoxParams],
  );
  /** 取得案號 */
  const caseno = useSelector((state: AppState) => state.userFile.userFile);

  /** 取得案名 */
  const casename = useSelector((state: AppState) => state.userFile.userFileNameOptions).filter((x) => {
    return x.value == caseno;
  })[0].text;
  /** 匯出保險箱資料excel */
  const exportBankSafeDepositBoxexcel = useCallback(() => {
    dispatch(exportExcelActionCreator({ ...currentFetchBankSafeDepositBoxParams }));
  }, [dispatch, currentFetchBankSafeDepositBoxParams]);

  /** 匯出保險箱資料ods */
  const exportBankSafeDepositBoxods = useCallback(() => {
    dispatch(exportOdsActionCreator({ ...currentFetchBankSafeDepositBoxParams }));
  }, [dispatch, currentFetchBankSafeDepositBoxParams]);

  /** 本案相關帳戶 */
  const handleIsAccountMark = useCallback(
    (seq, isAccountMark) => {
      dispatch(isAccountMarkActionCreator(seq, isAccountMark));
    },
    [dispatch],
  );

  /** 上傳CSV事件 */
  const handleUploadCsv = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      e.preventDefault();
      if (e.target.files && e.target.files.length) {
        const file = e.target.files[0];
        const formData = new FormData();
        formData.append('file', file);
        formData.append('caseno', caseno);
        formData.append('casename', casename);
        dispatch(parseCsvActionCreator(formData));
        e.target.value = '';
      }
    },
    [dispatch, caseno, casename],
  );

  /** Render DataTable Columns */
  const renderColumn: RenderColumnFunc<bankSafeDepositBox, bankSafeDepositBoxTableField> = ({ field, data }) => {
    if (field === 'isAccountMark' && data.isAccountMark) {
      return (
        <DataColumn>
          <div
            style={{ width: '30px', height: '30px', margin: 'auto', cursor: 'pointer', fill: 'rgb(11, 94, 215)' }}
            onClick={() => handleIsAccountMark(data.seq, data.isAccountMark)}
          >
            <svg
              className="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-hj1pze"
              focusable="false"
              aria-hidden="true"
              viewBox="0 0 24 24"
              data-testid="PersonSearchIcon"
            >
              <circle cx="10" cy="8" r="4"></circle>
              <path d="M10.35 14.01C7.62 13.91 2 15.27 2 18v2h9.54c-2.47-2.76-1.23-5.89-1.19-5.99zm9.08 4.01c.36-.59.57-1.28.57-2.02 0-2.21-1.79-4-4-4s-4 1.79-4 4 1.79 4 4 4c.74 0 1.43-.22 2.02-.57L20.59 22 22 20.59l-2.57-2.57zM16 18c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"></path>
            </svg>
          </div>
        </DataColumn>
      );
    } else if (field === 'isAccountMark') {
      return (
        <DataColumn>
          <div
            style={{ width: '30px', height: '30px', margin: 'auto', cursor: 'pointer', fill: 'rgb(85, 85, 85)' }}
            onClick={() => handleIsAccountMark(data.seq, data.isAccountMark)}
          >
            <svg
              className="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-suy6hi"
              focusable="false"
              aria-hidden="true"
              viewBox="0 0 24 24"
              data-testid="RemoveIcon"
            >
              <path d="M19 13H5v-2h14v2z"></path>
            </svg>
          </div>
        </DataColumn>
      );
    }

    return <DataColumn>{data[field]}</DataColumn>;
  };

  //#region Effects
  useEffect(() => {
    // 載入畫面時，取得帳號列表
    dispatch(fetchBankSafeDepositBoxsActionCreator(currentFetchBankSafeDepositBoxParams));

    return () => {
      // 重置頁面 State
      dispatch(resetPageStateActionCreator());
    };
  }, [dispatch]);

  //#endregion Effects

  return (
    <PageContainer>
      <PageHeader>
        <PageTitle>保險箱資料</PageTitle>
      </PageHeader>

      {/* Search Card */}
      <SearchCard>
        <form className="container-fluid" onSubmit={handleSearchFormSubmit}>
          <div className="row">
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="idCardNumber">身分證字號</Label>
                <Input
                  type="text"
                  id="idCardNumber"
                  name="idCardNumber"
                  onChange={(e) => updateFormData({ idCardNumber: e.target.value })}
                  value={formData.idCardNumber}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="renter">承租人姓名</Label>
                <Input
                  type="text"
                  id="renter"
                  name="renter"
                  onChange={(e) => updateFormData({ renter: e.target.value })}
                  value={formData.renter}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="mobilePhone">行動電話</Label>
                <Input
                  type="text"
                  id="mobilePhone"
                  name="mobilePhone"
                  onChange={(e) => updateFormData({ mobilePhone: e.target.value })}
                  value={formData.mobilePhone}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="boxNumber">箱號或室號</Label>
                <Input
                  type="text"
                  id="boxNumber"
                  name="boxNumber"
                  onChange={(e) => updateFormData({ boxNumber: e.target.value })}
                  value={formData.boxNumber}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="rentDateStart">承租日期(起)</Label>
                <DatePicker
                  id="rentDateStart"
                  name="rentDateStart"
                  onChange={(e) => updateFormData({ rentDateStart: e })}
                  value={formData.rentDateStart}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="rentDateEnd">承租日期(迄)</Label>
                <DatePicker
                  id="rentDateEnd"
                  name="rentDateEnd"
                  onChange={(e) => updateFormData({ rentDateEnd: e })}
                  value={formData.rentDateEnd}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="leaseCancellationDateStart">退租日期(起)</Label>
                <DatePicker
                  id="leaseCancellationDateStart"
                  name="leaseCancellationDateStart"
                  onChange={(e) => updateFormData({ leaseCancellationDateStart: e })}
                  value={formData.leaseCancellationDateStart}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="leaseCancellationDateEnd">退租日期(迄)</Label>
                <DatePicker
                  id="leaseCancellationDateEnd"
                  name="leaseCancellationDateEnd"
                  onChange={(e) => updateFormData({ leaseCancellationDateEnd: e })}
                  value={formData.leaseCancellationDateEnd}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-3">
              <FormGroup>
                <Label htmlFor="keyWord">關鍵字查詢</Label>
                <Input
                  type="text"
                  id="keyWord"
                  name="keyWord"
                  onChange={(e) => updateFormData({ keyWord: e.target.value })}
                  value={formData.keyWord}
                  disabled={isFetchBankSafeDepositBoxsLoading}
                />
              </FormGroup>
            </div>
          </div>
          <div className="row">
            <div className="col-12 text-center">
              <div className="input-group">
                <span className="input-group-text" id="basic-addon1">
                  只顯示本案相關帳戶
                </span>
                <span className="input-group-text">
                  <input
                    aria-label="Checkbox for following text input"
                    type="checkbox"
                    className="form-check-input"
                    id="isAccountMark"
                    name="isAccountMark"
                    onChange={(e) => updateFormData({ isAccountMark: e.target.checked ? '1' : '0' })}
                  />
                </span>
              </div>
              <button type="submit" className="btn btn-primary btn-width-xlg" disabled={isFetchBankSafeDepositBoxsLoading}>
                保險箱查詢
              </button>
            </div>
          </div>
        </form>
      </SearchCard>

      {/* Table Card */}
      <Card>
        <Card.Header>
          <PageSizeSelect options={PAGE_SIZE_OPTIONS} onChange={handlePageSizeChange} value={String(paginatedInfo.pageSize)} />
          <div>
            <input id="uploadFile" type="file" className="btn-check" onChange={handleUploadCsv}></input>
            <label className="btn btn-primary btn-width-lg" htmlFor="uploadFile" style={{ marginRight: '15px' }}>
              <i className="fas fa-upload"></i> .CSV資料匯入
            </label>
            <img
              src="data:image/svg+xml;base64,"
              className="jsx-2310615273"
              onClick={exportBankSafeDepositBoxexcel}
              style={{ width: '40px', height: '40px', marginRight: '10px', cursor: 'pointer' }}
            ></img>
            <img
              src="data:image/svg+xml;base64,"
              className="jsx-2310615273"
              onClick={exportBankSafeDepositBoxods}
              style={{ width: '40px', height: '40px', marginRight: '10px', cursor: 'pointer' }}
            ></img>
          </div>
        </Card.Header>
        <Card.Body>
          <DataTable
            columns={dataTableColumns}
            data={bankSafeDepositBox}
            onSortChange={handleSortedColumnChange}
            sortedColumn={currentFetchBankSafeDepositBoxParams.sortedColumn}
            sortedType={currentFetchBankSafeDepositBoxParams.sortedType}
            renderColumn={renderColumn}
            keyExtractor={(item: bankSafeDepositBox) => item.seq}
            isLoading={isFetchBankSafeDepositBoxsLoading}
          />
        </Card.Body>
        <Card.Footer>
          <Pagination
            page={paginatedInfo.page}
            totalPage={paginatedInfo.totalPage}
            pageSize={paginatedInfo.pageSize}
            totalCount={paginatedInfo.totalCount}
            onPaginatedButtonClick={handlePageChange}
            disabled={isFetchBankSafeDepositBoxsLoading}
          />
        </Card.Footer>
      </Card>
    </PageContainer>
  );
};
