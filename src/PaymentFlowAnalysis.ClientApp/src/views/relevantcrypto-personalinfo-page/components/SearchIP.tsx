...
/** DataTable 欄位名稱 */
export type RelevantCryptoPersonalInfoIPTableField = keyof PersonalIPShowModalData;
...
export default function SearchIPModal(props: any) {
  ...
  const CryptoPersonalSearchIPParams = useSelector(
    (state: AppState) => state.pages.RelevantCryptoPersonalInfo.CryptoPersonalSearchIPParams,
  );

  /** IP資料 */
  const IPData = useSelector((state: AppState) => state.pages.RelevantCryptoPersonalInfo.IPData);

  /** 分頁資訊資料 */
  const paginatedInfo = useSelector((state: AppState) => state.pages.RelevantCryptoPersonalInfo.paginatedIPInfo);
  ...
  const renderColumn: RenderColumnFunc<PersonalIPShowModalData, RelevantCryptoPersonalInfoIPTableField> = ({ field, data }) => {
