...
/** DataTable 欄位名稱 */
export type RelevantCryptoPersonalInfoPhoneTableField = keyof PersonalPhoneShowModalData;
...
export default function SearchPhoneModal(props: any) {
  ...
  const CryptoPersonalSearchPhoneParams = useSelector(
    (state: AppState) => state.pages.RelevantCryptoPersonalInfo.CryptoPersonalSearchPhoneParams,
  );

  /** 電話資料 */
  const phoneData = useSelector((state: AppState) => state.pages.RelevantCryptoPersonalInfo.phoneData);

  /** 分頁資訊資料 */
  const paginatedInfo = useSelector((state: AppState) => state.pages.RelevantCryptoPersonalInfo.paginatedPhoneInfo);
  ...
  const renderColumn: RenderColumnFunc<PersonalPhoneShowModalData, RelevantCryptoPersonalInfoPhoneTableField> = ({
    field,
    data,
  }) => {
