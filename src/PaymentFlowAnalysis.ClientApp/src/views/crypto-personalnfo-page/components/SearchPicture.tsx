...
/** DataTable 欄位名稱 */
export type CryptoPersonalInfoPictureTableField = keyof PersonalPictureShowModalData;
...
export default function SearchPictureModal(props: any) {
  ...
  const CryptoPersonalSearchPictureParams = useSelector(
    (state: AppState) => state.pages.CryptoPersonalInfo.CryptoPersonalSearchPictureParams,
  );

  /** Picture資料 */
  const PictureData = useSelector((state: AppState) => state.pages.CryptoPersonalInfo.PictureData);

  /** 分頁資訊資料 */
  const paginatedInfo = useSelector((state: AppState) => state.pages.CryptoPersonalInfo.paginatedPictureInfo);
  ...
  const renderColumn: RenderColumnFunc<PersonalPictureShowModalData, CryptoPersonalInfoPictureTableField> = ({
    field,
    data,
  }) => {
    if (field === 'subPath') {
      return (
        <DataColumn>
          <img className="card-img-top" src={data.subPath} style={{ width: '128px', height: '128px', textAlign: 'center' }} />
        </DataColumn>
      );
    }
