import { Card } from '@app/components/card';
import {
  DataColumn,
  DataTable,
  PageSizeSelect,
  PageSizeSelectChangeFunc,
  PaginatedButtonClickFunc,
  Pagination,
  RenderColumnFunc,
  SortChangeFunc,
} from '@app/components/data-table';
import { FormGroup, Input, Label } from '@app/components/form';
import { PageContainer, PageHeader, PageTitle } from '@app/components/page';
import { SearchCard } from '@app/components/search-card';
import { useForm } from '@app/hooks';
import { AppState } from '@app/store';
import { Alert } from '@modules/alert';
import { useNavigation } from '@modules/router';
import { DEFAULT_PAGE, PAGE_SIZE_OPTIONS } from '@shared/constants';
import { BlackAccount } from '@shared/types';
import React, { FormEvent, useCallback, useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import {
  exportExcelActionCreator,
  exportOdsActionCreator,
  fetchBlackAccountsActionCreator,
  hideModalActionCreator,
  initPageActionCreator,
  resetPageStateActionCreator,
  showCreateModalActionCreator,
  showEditModalActionCreator,
  showEmailListActionCreator,
  showIPListActionCreator,
  showPhoneListActionCreator,
} from './actions';
import { ModalForm } from './components/ModalForms';

/** DataTable 欄位名稱 */
export type BlackAccountTableField = keyof (BlackAccount & {
  actions: string;
});

/** SearchCard Form */
export type SearchCardForm = {
  /** 身分證字號 */
  idCardNum: string;
  /** 錢包地址 */
  walletAddress: string;
  /** 電話號碼 */
  phone: string;
  /** 電子信箱 */
  email: string;
  /** ip位置 */
  iP: string;
  /** 網址 */
  url: string;
  /** 備註 */
  remark: string;
};

/** 「模板頁面」元件 */
export const BlackAccountsPage = () => {
  //#region Hooks

  /** DataTable 欄位設定 */
  const [dataTableColumns] = useState([
    { field: 'actions', name: '編輯', isSortable: false },
    { field: 'exchangeTypeCodeStr', name: '資料來源', isSortable: false },
    { field: 'risklevelStr', name: '風險類別', isSortable: false },
    { field: 'idCardNum', name: '身分證字號' },
    { field: 'currencyType', name: '錢包幣別' },
    { field: 'walletAddress', name: '錢包地址', isPrimary: true },
    { field: 'userPhone', name: '電話號碼' },
    { field: 'userEmail', name: '電子信箱' },
    { field: 'userIP', name: 'ip位置' },
    { field: 'url', name: '網址' },
    { field: 'remark', name: '備註' },
    { field: 'updateTime', name: '資料修改時間' },
  ]);

  /** SearchCard Form 欄位資料 */
  const { formData, updateFormData } = useForm<SearchCardForm>({
    idCardNum: { initialValue: '', validate: () => ({}) },
    walletAddress: { initialValue: '', validate: () => ({}) },
    phone: { initialValue: '', validate: () => ({}) },
    email: { initialValue: '', validate: () => ({}) },
    iP: { initialValue: '', validate: () => ({}) },
    url: { initialValue: '', validate: () => ({}) },
    remark: { initialValue: '', validate: () => ({}) },
  });

  /** 當前取得帳號資料的搜尋條件資料 */
  const currentFetchBlackAccountsParams = useSelector(
    (state: AppState) => state.pages.blackAccounts.currentFetchBlackAccountsParams,
  );

  /** 帳號資料 */
  const blackAccounts = useSelector((state: AppState) => state.pages.blackAccounts.blackAccounts);

  /** 分頁資訊資料 */
  const paginatedInfo = useSelector((state: AppState) => state.pages.blackAccounts.paginatedInfo);

  /** 是否正在取得帳號資料 */
  const isFetchBlackAccountsLoading = useSelector((state: AppState) => state.pages.blackAccounts.isFetchBlackAccountsLoading);

  /** 新增帳號是否成功 */
  const isCreateBlackAccountSuccess = useSelector((state: AppState) => state.pages.blackAccounts.isCreateBlackAccountSuccess);

  /** 更新帳號是否成功 */
  const isUpdateBlackAccountSuccess = useSelector((state: AppState) => state.pages.blackAccounts.isUpdateBlackAccountSuccess);

  /** 是否刪除成功 */
  const isDeleteBlackAccountSuccess = useSelector((state: AppState) => state.pages.blackAccounts.isDeleteBlackAccountSuccess);

  /** Redux dispatch function */
  const dispatch = useDispatch();

  /** Navigation */
  const navigation = useNavigation();

  //#endregion Hooks

  /** 搜尋按鈕事件 */
  const handleSearchFormSubmit = useCallback(
    (e: FormEvent) => {
      e.preventDefault();
      dispatch(
        fetchBlackAccountsActionCreator({
          ...currentFetchBlackAccountsParams,
          ...formData,
          page: DEFAULT_PAGE,
        }),
      );
    },
    [dispatch, formData, currentFetchBlackAccountsParams],
  );

  /** 處理分頁大小變換事件 */
  const handlePageSizeChange: PageSizeSelectChangeFunc = useCallback(
    (pageSize) => {
      dispatch(
        fetchBlackAccountsActionCreator({
          ...currentFetchBlackAccountsParams,
          page: DEFAULT_PAGE,
          pageSize: Number(pageSize),
        }),
      );
    },
    [dispatch, currentFetchBlackAccountsParams],
  );

  /** 處理 DataTable 欄位排序變換事件 */
  const handleSortedColumnChange: SortChangeFunc = useCallback(
    (field, sortedType) => {
      dispatch(
        fetchBlackAccountsActionCreator({
          ...currentFetchBlackAccountsParams,
          page: DEFAULT_PAGE,
          sortedType,
          sortedColumn: field,
        }),
      );
    },
    [dispatch, currentFetchBlackAccountsParams],
  );

  /** 匯出定期接收帳戶資料excel */
  const exportBlackAccountexcel = useCallback(() => {
    dispatch(exportExcelActionCreator({ ...currentFetchBlackAccountsParams }));
  }, [dispatch, currentFetchBlackAccountsParams]);

  /** 匯出定期接收帳戶資料資料ods */
  const exportBlackAccountods = useCallback(() => {
    dispatch(exportOdsActionCreator({ ...currentFetchBlackAccountsParams }));
  }, [dispatch, currentFetchBlackAccountsParams]);

  /** 處理 DataTable 換頁事件 */
  const handlePageChange: PaginatedButtonClickFunc = useCallback(
    (nextPage) => {
      dispatch(
        fetchBlackAccountsActionCreator({
          ...currentFetchBlackAccountsParams,
          page: nextPage,
        }),
      );
    },
    [dispatch, currentFetchBlackAccountsParams],
  );

  /** 處理新增帳號 */
  const handleCreateEvent = useCallback(() => {
    dispatch(showCreateModalActionCreator());
  }, [dispatch]);

  /** 處理編輯事件 */
  const handleEditEvent = useCallback(
    (walletAddress) => {
      dispatch(showEditModalActionCreator(walletAddress));
    },
    [dispatch],
  );

  /** 顯示電話列表 */
  const showPhoneList = useCallback(
    (walletAddress) => {
      dispatch(showPhoneListActionCreator(walletAddress));
    },
    [dispatch],
  );

  /** 顯示信箱列表 */
  const showEmailList = useCallback(
    (walletAddress) => {
      dispatch(showEmailListActionCreator(walletAddress));
    },
    [dispatch],
  );

  /** 顯示ip列表 */
  const showIPList = useCallback(
    (walletAddress) => {
      dispatch(showIPListActionCreator(walletAddress));
    },
    [dispatch],
  );

  /** Render DataTable Columns */
  const renderColumn: RenderColumnFunc<BlackAccount, BlackAccountTableField> = ({ field, data }) => {
    if (field === 'actions' && data.exchangeTypeCode == '5') {
      return (
        <DataColumn>
          <button type="button" className="btn btn-warning btn-sm me-2" onClick={() => handleEditEvent(data.walletAddress)}>
            <i className="fas fa-pen" />
          </button>
        </DataColumn>
      );
    } else if (field === 'actions') {
      return <DataColumn></DataColumn>;
    }
    if (field === 'userPhone') {
      return (
        <DataColumn>
          <a style={{ color: 'var(--bs-link-color)', cursor: 'pointer' }} onClick={() => showPhoneList(data.walletAddress)}>
            {data.userPhone.split(',').length > 2
              ? data.userPhone.split(',')[0] + ',' + data.userPhone.split(',')[1] + '...'
              : data.userPhone}
          </a>
        </DataColumn>
      );
    } else if (field === 'userEmail') {
      return (
        <DataColumn>
          <a style={{ color: 'var(--bs-link-color)', cursor: 'pointer' }} onClick={() => showEmailList(data.walletAddress)}>
            {data.userEmail.split(',').length > 2
              ? data.userEmail.split(',')[0] + ',' + data.userEmail.split(',')[1] + '...'
              : data.userEmail}
          </a>
        </DataColumn>
      );
    } else if (field === 'userIP') {
      return (
        <DataColumn>
          <a style={{ color: 'var(--bs-link-color)', cursor: 'pointer' }} onClick={() => showIPList(data.walletAddress)}>
            {data.userIP.split(',').length > 2
              ? data.userIP.split(',')[0] + ',' + data.userIP.split(',')[1] + '...'
              : data.userIP}
          </a>
        </DataColumn>
      );
    }
    return <DataColumn>{data[field]}</DataColumn>;
  };

  //#region Effects
  useEffect(() => {
    // 載入畫面時，取得帳號列表
    dispatch(fetchBlackAccountsActionCreator(currentFetchBlackAccountsParams));
    // 初始頁面資料
    dispatch(initPageActionCreator());
    return () => {
      // 重置頁面 State
      dispatch(resetPageStateActionCreator());
    };
  }, [dispatch]);

  /** 新增成功，重新取得列表資料 */
  useEffect(() => {
    if (isCreateBlackAccountSuccess) {
      dispatch(hideModalActionCreator());
      dispatch(fetchBlackAccountsActionCreator(currentFetchBlackAccountsParams));
    }
  }, [isCreateBlackAccountSuccess, currentFetchBlackAccountsParams]);

  /** 更新成功，重新取得列表資料 */
  useEffect(() => {
    if (isUpdateBlackAccountSuccess) {
      dispatch(hideModalActionCreator());
      dispatch(fetchBlackAccountsActionCreator(currentFetchBlackAccountsParams));
    }
  }, [isUpdateBlackAccountSuccess, currentFetchBlackAccountsParams]);

  /** 刪除成功後，重新取得列表資料 */
  useEffect(() => {
    if (isDeleteBlackAccountSuccess) {
      dispatch(fetchBlackAccountsActionCreator(currentFetchBlackAccountsParams));
    }
  }, [isDeleteBlackAccountSuccess, currentFetchBlackAccountsParams]);

  //#endregion Effects

  return (
    <PageContainer>
      <PageHeader>
        <PageTitle>黑名單資料</PageTitle>
      </PageHeader>

      {/* Search Card */}
      <SearchCard>
        <form className="container-fluid" onSubmit={handleSearchFormSubmit}>
          <div className="row">
            <div className="col-12 col-sm-6 col-md-4">
              <FormGroup>
                <Label htmlFor="idCardNum">身分證字號</Label>
                <Input
                  type="text"
                  id="idCardNum"
                  name="idCardNum"
                  onChange={(e) => updateFormData({ idCardNum: e.target.value })}
                  value={formData.idCardNum}
                  disabled={isFetchBlackAccountsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-4">
              <FormGroup>
                <Label htmlFor="walletAddress">錢包地址</Label>
                <Input
                  type="text"
                  id="walletAddress"
                  name="walletAddress"
                  onChange={(e) => updateFormData({ walletAddress: e.target.value })}
                  value={formData.walletAddress}
                  disabled={isFetchBlackAccountsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-4">
              <FormGroup>
                <Label htmlFor="phone">電話號碼</Label>
                <Input
                  type="text"
                  id="phone"
                  name="phone"
                  onChange={(e) => updateFormData({ phone: e.target.value })}
                  value={formData.phone}
                  disabled={isFetchBlackAccountsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-4">
              <FormGroup>
                <Label htmlFor="email">電子信箱</Label>
                <Input
                  type="text"
                  id="email"
                  name="email"
                  onChange={(e) => updateFormData({ email: e.target.value })}
                  value={formData.email}
                  disabled={isFetchBlackAccountsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-4">
              <FormGroup>
                <Label htmlFor="iP">ip位置</Label>
                <Input
                  type="text"
                  id="iP"
                  name="iP"
                  onChange={(e) => updateFormData({ iP: e.target.value })}
                  value={formData.iP}
                  disabled={isFetchBlackAccountsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-4">
              <FormGroup>
                <Label htmlFor="url">網址</Label>
                <Input
                  type="text"
                  id="url"
                  name="url"
                  onChange={(e) => updateFormData({ url: e.target.value })}
                  value={formData.url}
                  disabled={isFetchBlackAccountsLoading}
                />
              </FormGroup>
            </div>
            <div className="col-12 col-sm-6 col-md-4">
              <FormGroup>
                <Label htmlFor="remark">備註</Label>
                <Input
                  type="text"
                  id="remark"
                  name="remark"
                  onChange={(e) => updateFormData({ remark: e.target.value })}
                  value={formData.remark}
                  disabled={isFetchBlackAccountsLoading}
                />
              </FormGroup>
            </div>
          </div>
          <div className="row">
            <div className="col-12 text-center">
              <button type="submit" className="btn btn-primary btn-width-xlg" disabled={isFetchBlackAccountsLoading}>
                黑名單查詢
              </button>
            </div>
          </div>
        </form>
      </SearchCard>

      {/* Table Card */}
      <Card>
        <Card.Header>
          <PageSizeSelect options={PAGE_SIZE_OPTIONS} onChange={handlePageSizeChange} value={String(paginatedInfo.pageSize)} />
          <div>
            <button
              type="button"
              className="btn btn-primary btn-width-lg"
              onClick={handleCreateEvent}
              style={{ marginRight: '15px' }}
            >
              新增黑名單
            </button>
            <img
              src="data:image/svg+xml;base64,"
              className="jsx-2310615273"
              onClick={exportBlackAccountexcel}
              style={{ width: '40px', height: '40px', marginRight: '10px', cursor: 'pointer' }}
            ></img>
            <img
              src="data:image/svg+xml;base64,"
              className="jsx-2310615273"
              onClick={exportBlackAccountods}
              style={{ width: '40px', height: '40px', marginRight: '10px', cursor: 'pointer' }}
            ></img>
          </div>
        </Card.Header>
        <Card.Body>
          <DataTable
            columns={dataTableColumns}
            data={blackAccounts}
            onSortChange={handleSortedColumnChange}
            sortedColumn={currentFetchBlackAccountsParams.sortedColumn}
            sortedType={currentFetchBlackAccountsParams.sortedType}
            renderColumn={renderColumn}
            keyExtractor={(item: BlackAccount) => item.walletAddress}
            isLoading={isFetchBlackAccountsLoading}
          />
        </Card.Body>
        <Card.Footer>
          <Pagination
            page={paginatedInfo.page}
            totalPage={paginatedInfo.totalPage}
            pageSize={paginatedInfo.pageSize}
            totalCount={paginatedInfo.totalCount}
            onPaginatedButtonClick={handlePageChange}
            disabled={isFetchBlackAccountsLoading}
          />
        </Card.Footer>
      </Card>
      <ModalForm></ModalForm>
    </PageContainer>
  );
};
