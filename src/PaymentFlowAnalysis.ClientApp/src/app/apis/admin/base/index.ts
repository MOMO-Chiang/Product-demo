import { httpRequest, HttpRequestOptions, HttpRequestError } from '@shared/http-request';
...
export const AdminHttpRequestFile = async <TData>(options: AdminHttpRequestOptions) => {
  ...
  const token = Auth.getToken();
  if (token) {
    baseHeaders = { ...baseHeaders, Authorization: `Bearer ${token}` };
  }
...
const appHttpDownload = async (url: string, data?: any, options?: AdminHttpRequestOptions) => {
  ...
  try {
    let filename: string = '';
    let header: string = response.headers['content-disposition'];
    if (header != '') {
      console.log('header:' + header);
      let startIndex = header.indexOf('filename=');
      if (startIndex > 0) {
        let endIndex = header.lastIndexOf('; filename');
        if (endIndex > 0) {
    ...
    exportElement.download = decodeURI(filename);
